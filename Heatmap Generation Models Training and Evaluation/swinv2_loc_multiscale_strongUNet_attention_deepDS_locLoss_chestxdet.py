...
Description
-----------
This script trains and evaluates a SwinV2-Large localization model with:
- Multi-scale feature fusion (32×32, 16×16, 8×8),
- Spatial attention on the deepest SwinV2 stage,
- A strong progressive UNet-lite decoder with deep supervision at 16×16 and 32×32,
- A rich localization loss (Focal BCE + Soft Dice + Lovasz IoU surrogate at 256×256,
  plus Focal BCE + Soft Dice at auxiliary 32×32 and 16×16 scales).
...
- Builds per-image:
    ...
    * 8-channel Gaussian heatmaps derived from bounding boxes (256×256).
- Uses a SwinV2-Large backbone (pretrained on MIMIC 14-class classification),
  adds a spatial attention block and a strong multi-scale UNet-lite decoder to
  predict 8 localization maps at 256×256 resolution with deep supervision.
- Trains with a joint objective:
    * Weighted BCE on 8-class labels (from 14-class logits).
    * Strong multi-scale localization loss with deep supervision.
...
Outputs
-------
- Strong multi-scale localization checkpoint:
    swinv2_loc_multiscale_strongUNet_attention_deepDS_locLoss_best.pth
...
- SwinV2 features from 32×32, 16×16, and 8×8 stages are fused via a strong
  multi-scale UNet-lite decoder with deep supervision and a rich localization loss.
"""
...
import timm
import matplotlib.pyplot as plt  # kept for possible debugging/visualization
...
# ===============================================================
# MAP_LOC_TO_14 (Index Mapping 8→14 classes for CheXlocalize)
# ---------------------------------------------------------------
# (Kept for compatibility; not directly used in this training loop.)
# ===============================================================
MAP_LOC_TO_14 = [
...
# ---------------------------------------------------------------
# Main 256×256:
#   Focal BCE + Soft Dice + Lovasz (IoU surrogate)
# Aux scales (32×32, 16×16):
#   Focal BCE + Soft Dice (weighted)
# ===============================================================
class FocalBCELoss(nn.Module):
    """
    Focal BCE for heatmap regression.
    Focuses training on hard / small regions.
    """
...
def soft_dice_loss(logits, targets, eps=1e-6):
    """
    Soft Dice loss for sparse multi-label heatmaps.
    """
...
def localization_loss_deep_supervision(seg256, aux16, aux32, gt256):
    """
    Strong localization loss with deep supervision.
    ...
    gt256  : [B,8,256,256] GT heatmaps

    Returns
    -------
    total_loc_loss : scalar
    """
    ...
    # Lovasz expects binary GT
    gt256_bin = (gt256 > 0.3).float()
    loss256 += lovasz_hinge(seg256, gt256_bin)
    ...
    # Weighted DS (main dominates)
    return loss256 + 0.5 * loss32 + 0.25 * loss16


# ===============================================================
# SwinV2 + SpatialAttention + Strong Multi-Scale UNet Decoder
# ---------------------------------------------------------------
# Outputs:
#   logits_14 : [B,14]
#   seg256    : [B,8,256,256]
...
class SpatialAttention(nn.Module):
    """Spatial attention to emphasize class-relevant regions."""
...
class MultiScaleSegDecoderStrong(nn.Module):
    """
    Strong multi-scale UNet-lite decoder.

    Inputs
    ------
    f32 : [B,384,32,32]   (shallow)
    f16 : [B,768,16,16]   (mid)
    f8  : [B,1536,8,8]    (deep)

    Returns
    -------
    seg256 : [B,8,256,256]   main heatmap logits
    aux16  : [B,8,16,16]     deep supervision logits
    aux32  : [B,8,32,32]     deep supervision logits
    """

    def __init__(self, c32=384, c16=768, c8=1536, out_ch=8):
        super().__init__()

        # 1×1 projections for skip alignment
        self.proj32 = nn.Conv2d(c32, 256, 1)   # 32×32 skip
        self.proj16 = nn.Conv2d(c16, 512, 1)   # 16×16 skip
        self.proj8  = nn.Conv2d(c8, 1024, 1)   # 8×8 deep

        # 8 → 16 upsample + fuse with 16×16 skip
        self.up8_to16 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(1024, 512, 3, padding=1),
            nn.ReLU(True),
        )
        self.aux16 = nn.Conv2d(512, out_ch, 1)  # DS @16

        # 16 → 32 upsample + fuse with 32×32 skip
        self.up16_to32 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(512, 256, 3, padding=1),
            nn.ReLU(True),
        )
        self.aux32 = nn.Conv2d(256, out_ch, 1)  # DS @32

        # 32 → 64 → 128 → 256 progressive refinement
        self.up32_to64 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(256, 128, 3, padding=1),
            nn.ReLU(True),
        )
        self.up64_to128 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(128, 64, 3, padding=1),
            nn.ReLU(True),
        )
        self.up128_to256 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
            nn.Conv2d(64, out_ch, 3, padding=1),
        )

        # Final edge refiner (light, but effective)
    ...
    def forward(self, f32, f16, f8):
        ...
        # Project skips / deep features
        f32 = self.proj32(f32)   # [B,256,32,32]
        f16 = self.proj16(f16)   # [B,512,16,16]
        f8  = self.proj8(f8)     # [B,1024,8,8]

        # 8 → 16 + skip
        x16 = self.up8_to16(f8)
        x16 = x16 + f16
        aux16 = self.aux16(x16)

        # 16 → 32 + skip
        x32 = self.up16_to32(x16)
        x32 = x32 + f32
        aux32 = self.aux32(x32)

        # 32 → 256 progressive
        x64 = self.up32_to64(x32)
        x128 = self.up64_to128(x64)
        seg256 = self.up128_to256(x128)
...
class SwinV2_AttentionSeg(nn.Module):
    """
    SwinV2 backbone + spatial attention + strong multi-scale decoder.

    Produces:
      - logits_14 : diagnosis logits (14-class)
      - seg256    : main 256×256 heatmaps (8 classes)
      - aux16     : DS @16×16
      - aux32     : DS @32×32
    """
    ...
    def __init__(self, backbone_name="swinv2_large_window12to16_192to256"):
        ...
        self.decoder = MultiScaleSegDecoderStrong(
            c32=384, c16=768, c8=1536, out_ch=len(LOC_CLASSES)
        )

        # Diagnosis head from deep stage
    ...
    def forward(self, x):
        ...
        # timm may return NHWC → normalize to NCHW
        def to_nchw(f):
...
# 1) Load backbone weights (patch_embed, layers, norm)
backbone_state = {}
...
# Losses + Optimizer
# ===============================================================
# Localization is harder, give it more weight
LAMBDA_LOC = 3.0
...
def train_one_epoch(model, loader):
    ...
    cls_loss:
      Weighted BCE over 8 supervised diagnosis labels.
    loc_loss:
      Focal BCE + Soft Dice + Lovasz @256
      + weighted deep supervision @32,16.
    """
    model.train()
    total_loss = 0.0
    ...
    for batch in loop:
        ...
        # ---- classification loss on 8 mapped labels ----
...
# ===============================================================
# Training Loop (saves swinv2_loc_multiscale_strongUNet_attention_deepDS_locLoss_best.pth)
# ===============================================================
EPOCHS = 20
best_val = float("inf")
CKPT_PATH = "swinv2_loc_multiscale_strongUNet_attention_deepDS_locLoss_best.pth"
...
print("✅ Loaded strong multi-scale attention localization model for evaluation.")
