...
Description
-----------
This script trains and evaluates a multi-scale attention-based SwinV2-Large
localization model on the ChestX-Det dataset, supervised on 8 localization
classes.
...
- Uses a SwinV2-Large backbone (pretrained on MIMIC 14-class classification),
  adds a spatial attention block on the deepest feature map and a multi-scale
  UNet-lite decoder with skip connections from 32×32, 16×16, and 8×8 stages to
  predict 8 localization maps at 256×256 resolution.
...
- Multi-scale attention-based localization checkpoint:
    swinv2_loc_multiscale_unetlite_attention_best.pth
...
- SwinV2 features are refined via a spatial attention block on the deepest
  8×8 stage, then decoded with a multi-scale UNet-lite decoder using
  32×32 and 16×16 skip connections to 8×256×256 localization maps.
"""
...
# ===============================================================
# SwinV2 Backbone + Spatial Attention + Multi-Scale Seg Decoder
# ---------------------------------------------------------------
# Outputs:
#   logits_14 : [B, 14]
#   loc_maps  : [B, 8, 256, 256]
# Notes:
# - Uses multi-scale SwinV2 features (32×32, 16×16, 8×8)
# - Fuses shallow + deep features for sharper localization.
# ===============================================================
...
class MultiScaleSegDecoder(nn.Module):
    """
    Multi-scale UNet-lite decoder with skip connections.

    Inputs
    ------
    f32 : [B,C32,32,32]   (shallow)
    f16 : [B,C16,16,16]   (mid)
    f8  : [B,C8 , 8, 8]   (deep)
    ...
    def __init__(self, c32=384, c16=768, c8=1536, out_ch=8):
        ...
        # 1×1 adapters to align skip channels
        self.proj32 = nn.Conv2d(c32, 256, 1)
        self.proj16 = nn.Conv2d(c16, 512, 1)
        self.proj8 = nn.Conv2d(c8, 1024, 1)

        # Upsample deep → 16×16 and fuse with mid
        ...
        # Upsample fused 16×16 → 32×32 and fuse with shallow
        ...
        # Final refinement and scale-up to 256×256
    ...
    def forward(self, f32, f16, f8):
        # ---- project to unified widths ----
        f32 = self.proj32(f32)   # [B,256,32,32]
        f16 = self.proj16(f16)   # [B,512,16,16]
        f8 = self.proj8(f8)      # [B,1024,8,8]

        # ---- 8→16 + fuse mid skip ----
        x = self.up8_to16(f8)    # [B,512,16,16]
        x = x + f16              # skip fusion

        # ---- 16→32 + fuse shallow skip ----
        x = self.up16_to32(x)    # [B,256,32,32]
        x = x + f32              # skip fusion

        # ---- 32→256 refinement ----
        x = self.up32_to64(x)    # [B,128,64,64]
        x = self.up64_to128(x)   # [B,64,128,128]
        x = self.up128_to256(x)  # [B,8,256,256]

        return x


class SwinV2_MultiScaleAttentionSeg(nn.Module):
    """
    ...
    def __init__(self, backbone_name="swinv2_large_window12to16_192to256"):
        ...
        # SwinV2 multi-scale features: 32×32, 16×16, 8×8
        ...
        # Attention on deepest stage only
        self.att = SpatialAttention(1536)

        # Multi-scale decoder with skip connections
        self.decoder = MultiScaleSegDecoder(
            c32=384, c16=768, c8=1536, out_ch=len(LOC_CLASSES)
        )

        # Diagnosis head (14 classes) from deepest refined features
    ...
    def forward(self, x):
        ...
        f8_att = self.att(f8)     # [B,1536,8,8]

        # Multi-scale decoder (uses skips)
        loc_maps = self.decoder(f32, f16, f8_att)  # [B,8,256,256]

        # Diagnosis logits from deep features
        pooled = self.pool(f8_att).flatten(1)      # [B,1536]
...
model = SwinV2_MultiScaleAttentionSeg(backbone_name).to(DEVICE)
...
# ===============================================================
# Training Loop (saves swinv2_loc_multiscale_unetlite_attention_best.pth)
# ===============================================================
EPOCHS = 20
best_val = float("inf")
CKPT_PATH = "swinv2_loc_multiscale_unetlite_attention_best.pth"
...
# ===============================================================
model = SwinV2_MultiScaleAttentionSeg().to(DEVICE)

state = torch.load(CKPT_PATH, map_location=DEVICE)
model.load_state_dict(state, strict=True)
model.eval()

print("✅ Loaded multi-scale attention localization model for evaluation.")
    ...
    for batch in tqdm(loader):
        ...
        for b in range(pred_maps.shape[0]):
            ...
            for cid, cls_name in enumerate(LOC_CLASSES):
                ...
                gt_boxes = [
                    b for b, s in zip(meta["boxes_scaled"], meta["mapped_syms"])
                    if s == cls_name
                ]
