...
Description
-----------
This script trains and evaluates a 14-label Vision Transformer (ViT-Base)
multilabel chest X-ray classifier on a balanced MIMIC-CXR subset.

It:
- Loads image-level labels and DICOM metadata (AP/PA only)
- Builds train/val/test splits (70/15/15)
- Trains a ViT-Base (patch16, 384×384, grayscale) classifier with
  Weighted Binary Cross-Entropy loss
- Saves the best checkpoint according to validation ROC-AUC
- Reloads the best checkpoint and computes detailed test metrics:
  - Per-label ROC-AUC / PR-AUC / best F1 / best threshold
  - Global macro/micro F1, ROC-AUC, and ECE
  - Exports CSVs and a Top-5 PR curve figure
...
- mimic_cxr_balanced_subset_v2.csv
  Required columns:
    - subject_id
    - study_id
    - 14 CheXpert-style label columns (see `label_cols`)

- dcm_info.csv
  Required columns:
    - subject_id
    - study_id
    - image_path (relative, starting with "images")
...
-------
- Trained checkpoint:
    vit_14class_weightedbce.pth

- Evaluation artifacts (in ./evaluation_csvs):
    vit_14class_weightedbce_perclass.csv
    vit_14class_weightedbce_summary.csv
    vit_14class_weightedbce_top5_pr.png

Notes
-----
- Uses ViT Base (timm: "vit_base_patch16_384") with:
  - 1-channel grayscale input (in_chans=1)
  - A custom 14-class multilabel head.
- Pos-weights are estimated from the *training* labels only, clipped and
  normalized for numerical stability, and passed to a custom
  WeightedBCELoss.
- Augmentations (including grayscale, resize to 384×384) are applied only
  on the training split; val/test use plain resize + normalization.
"""
...
# ===============================================================
# Image Transforms & DataLoaders (Grayscale 384×384)
# ===============================================================
...
# ===============================================================
# Model: ViT-Base (grayscale, 384×384, 14-class head)
# ===============================================================
class CXRViT(nn.Module):
    """
    Vision Transformer backbone (ViT-Base, patch16, 384×384) with a
    custom 14-class multilabel classification head.

    - Uses in_chans=1 for grayscale CXRs.
    - Aggregates patch tokens via mean pooling.
    """
    ...
    def __init__(self, num_classes=14, pretrained=True):
        ...
        self.backbone = timm.create_model(
            ...
            in_chans=1,      # 1-channel grayscale input
            num_classes=0,   # remove ImageNet classifier head
        )
        in_features = self.backbone.embed_dim  # 768 for ViT-Base
        self.head = nn.Linear(in_features, num_classes)

    def forward(self, x):
        # forward_features returns [B, num_patches, embed_dim]
...
model = CXRViT(num_classes=NUM_CLASSES, pretrained=True).to(DEVICE)

# Sanity check
x_dummy = torch.randn(2, 1, 384, 384).to(DEVICE)
with torch.no_grad():
    y_dummy = model(x_dummy)
print("✅ ViT Output shape:", y_dummy.shape)
...
EPOCHS = 20
best_auc = 0.0
CKPT_PATH = "vit_14class_weightedbce.pth"
...
model_eval = CXRViT(num_classes=NUM_CLASSES, pretrained=False).to(DEVICE_EVAL)
state_dict = torch.load(CKPT_PATH, map_location=DEVICE_EVAL)
...
for _, row in results_df.head(5).iterrows():
    idx = label_cols.index(row["Label"])
    precision, recall, _ = precision_recall_curve(y_true[:, idx], preds[:, idx])
    plt.plot(recall, precision, label=f'{row["Label"]} (AUC={row["PR_AUC"]:.2f})')

plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("Top-5 PR Curves — Vision Transformer")
plt.legend()
plt.tight_layout()
