...
Description
-----------
This script trains and evaluates a 14-label BEiT v2 Large chest X-ray classifier
on a balanced MIMIC-CXR subset. It uses 1-channel (grayscale) frontal-view PNG
images linked via DICOM metadata, applies standard augmentations, and optimizes
a weighted Binary Cross-Entropy loss to correct for class imbalance.
...
Outputs
-------
- beitv2_14class_weightedbce.pth
  Fine-tuned BEiT v2 Large weights (14-class multilabel classifier).

- evaluation_csvs/beitv2_14class_weightedbce_perclass.csv
  Per-label ROC AUC, PR AUC, best F1, and best threshold.

- evaluation_csvs/beitv2_14class_weightedbce_summary.csv
  Global metrics: mean ROC/PR AUC, macro/micro F1, precision, recall,
  micro ROC AUC, and ECE.

- evaluation_csvs/beitv2_14class_weightedbce_top5_pr.png
  Precision–recall curves for the top-5 labels by PR AUC.

Notes
-----
- Uses BEiT v2 Large (timm: "beitv2_large_patch16_224") with:
    - 1-channel input (grayscale CXRs)
    - 14-class multilabel head (custom linear layer).
...
- Augmentations are applied only to the training split; val/test use
  plain resizing + normalization (same grayscale pipeline).
"""
...
# ===============================================================
# Image Transforms & DataLoaders (Grayscale, 1-channel)
# ===============================================================
...
# ===============================================================
# Model: BEiT v2 Large (14-class multilabel head)
# ===============================================================
NUM_CLASSES = 14


class BEiT2CXR(nn.Module):
    """
    BEiT v2 Large backbone + 14-class multilabel classification head.

    - Backbone: timm "beitv2_large_patch16_224"
      (in_chans=1, num_classes=0 to drop the original head)
    """
    ...
    def forward(self, x):
        # forward_features → [B, N_patches, embed_dim]
        x = self.backbone.forward_features(x)
        x = x.mean(dim=1)  # global average over patch tokens
        return self.head(x)


model = BEiT2CXR(num_classes=NUM_CLASSES).to(DEVICE)
...
EPOCHS = 20
best_auc = 0.0
CKPT_PATH = "beitv2_14class_weightedbce.pth"
...
class BEiT2CXR_Eval(nn.Module):
    """
    Same architecture as BEiT2CXR used during training, but with
...
model_eval = BEiT2CXR_Eval(num_classes=NUM_CLASSES).to(DEVICE)
...
plt.title("Top-5 PR Curves — BEiT v2 Large")
plt.legend()
plt.tight_layout()
