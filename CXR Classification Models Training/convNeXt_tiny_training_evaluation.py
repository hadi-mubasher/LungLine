...
Description
-----------
This script trains and evaluates a 14-class ConvNeXt-Tiny classifier on a
...
Outputs
-------
- convnext_tiny_best_14class_bce.pth
  Fine-tuned ConvNeXt-Tiny weights (14-class multilabel classifier).

- evaluation_csvs/convnext_tiny_best_14class_bce_perclass.csv
  Per-label ROC AUC, PR AUC, best F1, and best threshold.

- evaluation_csvs/convnext_tiny_best_14class_bce_summary.csv
  Global metrics: mean ROC/PR AUC, macro/micro F1, precision, recall,
  micro ROC AUC, and ECE.

- evaluation_csvs/convnext_tiny_best_14class_bce_top5_pr.png
  Precision–recall curves for the top-5 labels by PR AUC.

Notes
-----
- Uses ConvNeXt Tiny (timm: "convnext_tiny") with a 14-class classifier
  head and global average pooling.
...
# ===============================================================
# Model: ConvNeXt-Tiny (14-class multilabel head)
# ===============================================================
NUM_CLASSES = 14
...
with torch.no_grad():
    ...
    y_test = model(x_test)
print("✅ ConvNeXt-Tiny output shape:", y_test.shape)
...
EPOCHS = 20
best_auc = 0.0
CKPT_PATH = "convnext_tiny_best_14class_bce.pth"
...
plt.title("Top-5 PR Curves — ConvNeXt Tiny")
plt.legend()
plt.tight_layout()
